...
def anagram(input_string):
    ...
    while index <= length:
        for letter in word_string:
            for letter2 in word_string[index:length]:
                if letter == letter2:
                    continue
                else:
                    linked_list_swaps.swap(letter, letter2)
                    new_strings.append(linked_list_swaps.read())
...
if __name__ == '__main__':
    ...
    print(random_rearrange(input_string))
    print(reverse_order(input_string))
    # mad_libs()
    print(anagram(anagram_string))
